...
def get_files(path:PathOrStr, extensions:Collection[str]=None, recurse:bool=False,
    ...
    if recurse:
        ...
        if presort: 
            res = sorted(res, key=lambda p: _path_to_same_str(p), reverse=False)
        return res
    else:
...
class ItemList():
    ...
    def __init__(self, items:Iterator, path:PathOrStr='.', label_cls:Callable=None, inner_df:Any=None,
        ...
        if not isinstance(self.items,np.ndarray): 
            self.items = array(self.items, dtype=object)
    ...
    def _relative_item_path(self, i): 
        return self.items[i].relative_to(self.path)
    def _relative_item_paths(self):  
        return [self._relative_item_path(i) for i in range_of(self.items)]
